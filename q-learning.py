...
class Agent:
    ...
    def policy(self, s, actions):
        ...
        # random.random( )は0.0～1.0までのfloat値を取得
        # floatとは、 Pythonの組み込み型の1つで、浮動小数点を扱う
        # 浮動小数点数とは、誤差が生じてしまうもの、ものすごく小さい小数点
        if np.random.random() < self.epsilon:

                # epsilonの確率でランダムに行動
                # len(actions)で数値を0.1〜1.0を出力している
    ...
    # Q学習をする
    def learn(self, env, actions, episode_count=1, gamma=0.9, learning_rate=0.1):
    ...
    # テストバトル
    def test_run(self, env, actions, draw=True, episode_count=1):
...
if __name__ == "__main__":
    ...
    """" 完全ランダムでバトル """
    agent.epsilon = 1.0
    agent.test_run(game, actions, episode_count=1)

    """ Q学習する """
    agent.epsilon = 0.2
    agent.learn(game, actions, episode_count=1)
